...
### Bivariate Correlation Coefficients - Pearson product-moment correlation coefficient

<span style="display:block" id="math">
$r_{12} = \frac{\sum(Y_{i1} - \hat{Y_1})(Y_{i2} - \hat{Y_2})}{\sqrt{\sum(Y_{i1} - \hat{Y_1})^2 \sum(Y_{i2} - \hat{Y_2})^2}}$
</span>
...
```{r, message=FALSE}
# Read the dataset and convert it into a data.table
...
df62<-fread("data/AppendixCDataSets/APPENC02.txt", sep=" ", col.names=cnames)
 
 # Define the variability function
...
ans <- data.table(Predictor = nm1, R2 = numeric(length(nm1)))

# Loop through each predictor name and calculate R2
for (pred in nm1) {
    ans[Predictor == pred, R2 := variability(df62, pred)]
}

ans <- ans[order(-R2)]  # Order the rows by R2 in descending order
...
## (Problem 2.63) Refer to the CDI data set.  For each geographic region, regress per capita income against the percentage of individuals in a county having at least a bachelor's degree.  Obtain a separate interval estimate of $\beta_1$ for each region.  Use a 90 percent confidence coefficient in each case.  Do the regression lines for the different regions appear to have similar slopes?

```{r, message=FALSE}
# Read the dataset and convert it into a data.table

cnames<-c("ID","County","State","Area","Pop","Young","Old","Phys","Beds","Crime","HighSchool","BA","Poverty","Unemploy","PerCapitaIncome","TotalIncome","Region")
dt<-fread("data/AppendixCDataSets/APPENC02.txt", sep=" ", col.names=cnames)
...
## (Problem 2.47)  Refer to Muscle mas Problem 1.27.  Assume that the normal bivatiate model (2.74) is appropriate.

### Compute the Pearson product moment correlation coefficient $r_{12}$

### Test whether muscle mass and age are statisticall independent in the population; use $\alpha = .05$.  State th alternatives, decision rule, and conclusion.

### The bivariate normal model (2.74) assumption is possibly inappropriate here. Compute the Spearman rank correlation coefficient, $r_s$.

### Repeat part (b), this time basing the test of independence on the Spearman rank correlation computed in part (c) and test statistic (2.101).  Use $\alpha = .05$.  State the alternatives, decision rule, and conclusion.
