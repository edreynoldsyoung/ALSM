...
### d.  Verify that your fitted regression line goes through the point $(\hat{X}, \hat{Y})$
 ...
 $(\hat{X}, \hat{Y})=($  `r mean(df21$X)` , `r mean(df21$Y)`  $)$
 
 `r (m21$coefficients[1])+(m21$coefficients[2]*mean(df21$X))`
...
`r sum((m27$residuals)^2)/(length(m27$residuals)-2)`