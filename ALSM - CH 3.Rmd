...
### 1. The regression function is not linear (functional misspecification)
...
### 2. The error terms do not have constant variance (heteroskedacity)
...
### 3. The error terms are not independent (serial corelation of error terms)
...
### 5. The error terms are not normally distributed (structural changes in error terms)
...
### 1. Scatter plot of the Independent Variable against the Dependent Variable with a lowess (loess) Fit Line.
### 2. Plot of residuals against predictor variable
### 3. Plot of absolute or squared residuals against predictor variable and fitted values
### 4. Plot of residuals against fitted values
### 5. Plot of residuals against time or other sequence
### 6. Plots of residuals against omitted predictor variables
### 6. Box plot, stem-and-leaf plots, and dot plots of residuals
### 7. Normal probability plot of residuals

## Statistical Tests

### 1. Runs Test
### 2. Correlation between the ordered residuals and their expected values
### 2. Durbin-Watson Test
### 3. Brown-Forsythe Test
### 4. Breusch-Pagan Test
### 5. Hat Matrix (see Ch 10)
### 6. Chi-Square test
### 7. Kolmogorov-Smirnov Test
### 8. Lilliefors Test
### 9. Lack of Fit Test (F Test)

## Remedial Measures

### 1. Abandon Linear Regression Model and develop more appropriate model
#### a. Non Linear Model
#### b. Multiple Regression Model
#### c. Robust Regression Models
#### d. Weighted Least Squares
#### e. Correlated Error Models
### 2. Transformations
...
#### c. Shape of Regression Function
##### i. moving averages
##### ii. running medians
##### iii. Lowess
...
<span style="display:block" id="note"> Semi-studentized Residuals</span>

<span style="display:block" id="math">$e_i^* = \frac{e_i}{\sqrt{MSE}}$</span>

****

<span style="display:block" id="note"> Normal Probability Plot of residuals</span>
...
<span style="display:block" id="note"> Brown-Forsythe Test for constancy of error variance</span>
<span style="display:block" id="note"> Devide the ample into two groups:</span>
...
<span style="display:block" id="note"> Breusch-Pagan Test for constancy of error variance</span>

<span style="display:block" id="math">
$log_e \sigma_i^2 = \gamma_0 + \gamma_1 X_i\\
\textrm{Constancy of error variance corresponds to }\gamma_1=0\\
...
## (Problem 3.4) Refer to **Copier maintenance** Problem 1.20
...
### a. Prepare a dot plot for the number of copiers serviced $X_i$.  What information is provided by this plot?  Are there an outlying cases with respect to this variable?
...
### d. Prepare residual plots of $e_i$ versus $\hat{Y}$ and $e_i$ versus $X_i$ on separate graphs.  Do these plots provide the same information?  What departures from regression model (2.1) can be studied from these plots?  State your findings.
...
### e. Prepare a normal probability plot of the residuals.  Also obtain the coefficient of correlation between the ordered residuals and their expected values under normality.  does the normality assumption appear to be tenable here?  Use $\alpha=.05$
...
### g. Assume that $log_e \sigma_i^2 = \gamma_0 + \gamma_1 X_i$  Conduct the Breusch-Pagan test to determine whether or not the error variance varies with the level of $X$.  Use $\alpha = .05$  State the alternatives, decision rule, and conclusion.
...
Because $|X_{BP}^2|<\chi^2(.95; 1)$ I conclude $H_0$ that the error variance is constant with levels of X.

## (Problem 3.5) Refer to **Airfreight breakage** Problem 1.21
...
### a. Prepare a dot plot for the number of transfers $X_i$.  Does the distribution of the number of tranfers appear to be asymmetrical?
...
### b. The cases are given in time order.  Prepare a time plot for the number of tranfers.  Is any systematic pattern evident in your plot?  Discuss.
...
### e. Prepare a normal probability plot of the residuals.  Also obtain the coefficeint of corretaion between the ordered residuals and their expected values under normality to ascertain whether the normality assumption is reasonable here.  Use $\alpha=.01$.  What do you conclude?
...
### g. Conduct the Breusch-Pagan test to determine whether or not the error variance varies with the level of $X$.  Use $\alpha=.10$.  State the alternatives, decision rule, and conclusion.  Does your conclusion support your prelimiary findings in part (d)?
...
Because $|X_{BP}^2|\leq\chi^2(.90; 1)$ I conclude $H_0$ that the error variance is constant with levels of X.

## (Problem 3.7) Refer to **Muscle mass** Problem 1.27
...
### c. Plot the residuals $e_i$ against $\hat{y}_i$ and also against $X_i$ on separate graphs to ascertain whether any departures from regression model 2.1 are evident.  Do th two plots provide the same information?  State your conclusion.
...
### d. Prepare a normal probability plot of the residuals.  Also obtain the coefficient of correlation between the ordered residuals and their expected values under normality to ascertain whether the normality assumption is tenable here.  Use $\alpha=.10$
...
### e. Conduct the Breusch-Pagan test to determine whether or not the error variance varies with the level of $X$.  Use $\alpha=.01$. State the alternatives, decision rule, and conclusion.  Is your conclusion consistent with your preliminary findings in part (c)?


## (Problem 3.13) Refer to **Copier maintenance** Problem 1.20

### a. What are the alternative conclusion when testing for lack of fit of a linear regression funtion?

### b. Perform the test indicated in part (a).  Control the risk of Type I error at .05.  State the decion rule and conlcusion.

### c. Does the test in part (b) detect other departures from regression model 2.1, such as lack of constant variance or lack or normality in the error terms?  Could the results of the test of lack of fit be affected by such departures?  Discuss.


## (Problem 3.17) **Sales growth**  A marketing researcher studied annual sales of a product that had beenintroduced 10 years ago.  The data are as follow, where $X$ is the year (coded) and $Y$ is sales in thousands of units.

### a. Prepare a scatter plot of the data.  Does a linear relation appear adequate here?

### b. Use the Box-Cox proceudre and standardization (3.36) to find an appropriate power transformation of $Y$.  Evaluate SSE for $\lambda =.3,.4,.5,.6,.7$.  What transformation of $Y$ is suggested?  

### c. Use the transformation $Y^t=\sqrt{Y}$ and obtain the estimated linear regression function for the transformed data.

### d. Plot the estimated regression line and the transformed data.  Does theregression line appear to be a good fit to the transformed data?

### e. Obtain the residuals and plot the against the fitted values.  Also prepare a normal probability plot.  What do your plots show?

### f. Express the estimated ression function in the original units.
